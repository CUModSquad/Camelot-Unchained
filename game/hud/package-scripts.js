// helper function to generate scripts for each target
function generateForTargets(callback) {
  const targets = [
    { name: 'fledgling', id: 30, },
    { name: 'hatchery', id: 4, },
    { name: 'wyrmling', id: 10, },
    { name: 'wyrmlingPrep', id: 11, },
    { name: 'nuada', id: 1300, },
    { name: 'nuadaPrep', id: 1400, },
    { name: 'wolfhere', id: 1100, },
    { name: 'cube', id: 27, },
  ];
  return generateFor(targets, callback);
}

// helper function to generate scripts
function generateFor(list, callback) {
  let scripts = {};
  list.forEach((item) => {
    Object.assign(scripts, callback(item));
  });
  return scripts;
}

module.exports = {
  scripts: {

    build: {
      default: {
        script: 'nps report.start && nps copy,build.webpack.production,build.sass,copy.dist && nps report.success',
        description: 'Builds the UI in production mode',
      },
      dev: {
        script: 'nps report.start && nps copy,build.webpack.development,build.sass,copy.dev && nps report.success',
        description: 'Builds the UI in development mode',
      },
      webpack: {
        default: {
          hiddenFromHelp: true,
          script: 'nps build',
        },
        development: {
          hiddenFromHelp: true,
          script: 'webpack --mode development',
        },
        production: {
          hiddenFromHelp: true,
          script: 'webpack --mode production',
        },
      },
      sass: {
        hiddenFromHelp: true,
        script: 'node-sass src/index.scss -o build/css --importer node_modules/sass-importer-node/sass-importer-node.js --quiet && nps report.sass',
      },
      ...generateForTargets((target) => {
        return {
          [target.name]: {
            default: {
              script: `nps build,clean.${target.name},copy.${target.name}`,
              description: `Builds the UI in production mode and copies to the ${target.name} (${target.id}) UI override directory.`,
            },
            dev: {
              script: `nps build.dev,clean.${target.name},copy.${target.name}`,
              description: `Builds the UI in development mode and copies to the ${target.name} (${target.id}) UI override directory.`,
            }
          },
        };
      }),
    },

    // Dev
    dev: {
      default: {
        script: 'nps clean && nps gql.codegen && nps gql.collectAndConcat && nps build.sass,copy.dev,copy.dist && nps dev.watch',
        description: 'Development mode will start an http server with live reload that will watch and build whenever a file change is detected.',
      },
      serve: {
        hiddenFromHelp: true,
        script: 'webpack-serve --content ./dist --open',
      },
      watch: {
        default: {
          hiddenFromHelp: true,
          script: 'nps -p dev.serve,dev.watch.graphql,dev.watch.sass,dev.watch.misc',
        },
        webpack: {
          hiddenFromHelp: true,
          script: 'webpack --mode development --watch',
        },
        graphql: {
          hiddenFromHelp: true,
          script: 'watch -p "src/**/*.graphql -c "nps gql.codegen && nps gql.collectAndConcat"',
        },
        sass: {
          hiddenFromHelp: true,
          script: 'watch -p "src/**/*.scss" -c "nps build.sass"',
        },
        misc: {
          hiddenFromHelp: true,
          script: 'watch -p "src/**/*.html" -p "src/third-party/**/*" -p "src/font/**/*" -p "src/images/**/*" -p "src/**/*.ui" -p "src/**/*.ico" -p "src/**/*.config.js" -c "nps copy.dev,copy.dist"',
        },
      },
      webpack: {
        default: {
          hiddenFromHelp: true,
          script: 'nps dev',
        },
        hatchery: {
          hiddenFromHelp: true,
          script: 'nps dev.hatchery',
        },
      },
      ...generateForTargets((target) => {
        return {
          [target.name]: {
            default: {
              script: `nps clean && nps gql.codegen && nps gql.collectAndConcat && nps build.sass && nps copy && nps copy.dist && nps clean.${target.name} && nps copy.${target.name} && nps dev.${target.name}.watch`,
            },
            watch: {
              hiddenFromHelp: true,
              script: `cross-env CUUI_DEV_OUTPUT_PATH="%localappdata%/CSE/CamelotUnchained/${target.id}/INTERFACE/hud" nps -p dev.watch.webpack,dev.watch.graphql,dev.watch.sass,dev.watch.misc`,
            }
          },
        };
      }),
    },

    // Deploy
    deploy: {
      script: 'cross-env CUUI_ENABLE_SENTRY="1" nps clean,build.webpack && rimraf ../../../CamelotUnchained/MMO/Client/Assets/interface/hud && copyup build/**/* ../../../CamelotUnchained/MMO/Client/Assets/interface/hud',
      description: 'Deploys a fresh build to the client assets directory, CamelotUnchained & CamelotUnchained-UI repositories should be side by side in the same root directory.'
    },

    // Clean
    clean: {
      default: {
        script: 'rimraf build && rimraf dist',
        hiddenFromHelp: true,
      },
      ...generateForTargets((target) => {
        return {
          [target.name]: {
            hiddenFromHelp: true,
            script: `rimraf \"%localappdata%/CSE/CamelotUnchained/${target.id}/INTERFACE/hud\"`,
          },
        };
      }),
    },

    // Copy
    copy: {
      dist: {
        script: 'copyup build/**/* dist && nps report.copy',
        hiddenFromHelp: true,
      },
      default: {
        script: 'copyup src/third-party/**/* src/images/**/* src/font/**/* src/**/*.ico src/**/*.ui build && nps report.copy',
        hiddenFromHelp: true,
      },
      dev: {
        script: 'nps copy && copyup src/**/*.config.js dist && nps report.copy',
        hiddenFromHelp: true,
      },
      ...generateForTargets((target) => {
        return {
          [target.name]: {
            hiddenFromHelp: true,
            script: `copyup build/**/* \"%localappdata%/CSE/CamelotUnchained/${target.id}/INTERFACE/hud\" && nps report.copy`,
          },
        };
      }),
    },


    // GraphQL
    ...generateFor(
      [
        { name: '', api: 'https://hatcheryapi.camelotunchained.com/graphql'},
        { name: 'Local', api: 'http://localhost:1337/graphql'},
        { name: 'LocalServer', api: 'http://localhost:8000/graphql'},
      ],
      (target) => {
        const name = `gql${target.name}`;
        return {
          [name]: {
            default: {
              script: `(nps ${name}.mkdir && nps ${name}.schema && nps ${name}.codegen && nps ${name}.collectAndConcat) || echo continuing...`,
            },
            mkdir: {
              hiddenFromHelp: true,
              script: 'if not exist "gql" mkdir gql',
            },
            schema: {
              hiddenFromHelp: true,
              script: `apollo-codegen introspect-schema ${target.api} --output gql/schema.json`,
            },
            codegen: {
              hiddenFromHelp: true,
              script: 'apollo-codegen generate src/**/*.graphql --schema gql/schema.json --target typescript --output src/gqlInterfaces.ts',
            },
            collectAndConcat: {
              hiddenFromHelp: true,
              script: 'graphql-document-collector "src/**/*.graphql" > gql/gqlDocument.json && concat-cli -f src/gqlPrepend.txt -f gql/gqlDocument.json -o src/gqlDocuments.ts',
            },
          }
        }
      }
    ),

    // Linting
    lint: {
      default: {
        script: 'tslint -t stylish src/**/*.ts{,x} && nps report.lint',
        description: 'Run TS-Lint"',
      },
      fix: {
        script: 'tslint --fix src/**/*.ts{,x}',
        description: 'Fix TS-Lint errors',
        hiddenFromHelp: true,
      }
    },

    // Reporting
    report: {
      start: {
        script: 'echo "Build started..."',
        hiddenFromHelp: true,
      },
      copy: {
        script: 'echo "Copy complete..."',
        hiddenFromHelp: true,
      },
      sass: {
        script: 'echo "SCSS compile complete..."',
        hiddenFromHelp: true,
      },
      success: {
        script: 'echo "Build completed successfully!"',
        hiddenFromHelp: true,
      },
      lint: {
        script: 'echo "TSLint complete"',
        hiddenFromHelp: true,
      },
      test: {
        script: 'echo "Testing started..."',
        hiddenFromHelp: true,
      },
      gql: {
        script: 'echo "Generating GraphQL Documents and Typings...',
        hiddenFromHelp: true,
      }
    },
  }
};
